...
public class ESDriver implements Serializable {
		...
		setBulkProcessor(BulkProcessor.builder(getClient(), new BulkProcessor.Listener() {
			...
			public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
				...
				throw new RuntimeException(
						"Caught exception in bulk: " + request.getDescription() + ", failure: " + failure, failure);
			}
		}).setBulkActions(1000).setBulkSize(new ByteSizeValue(2500500, ByteSizeUnit.GB))
	...
	public void deleteAllByQuery(String index, String type, QueryBuilder query) {
		ImmutableOpenMap<String, MappingMetaData> mappings = getClient().admin().cluster().prepareState().execute()
				.actionGet().getState().metaData().index(index).getMappings();
		...
		createBulkProcessor();
		SearchResponse scrollResp = getClient().prepareSearch(index).setSearchType(SearchType.QUERY_AND_FETCH)
				.setTypes(type).setScroll(new TimeValue(60000)).setQuery(query).setSize(10000).execute().actionGet();
		...
		while (true) {
			...
			scrollResp = getClient().prepareSearchScroll(scrollResp.getScrollId()).setScroll(new TimeValue(600000))
					.execute().actionGet();
	...
	public String searchByQuery(String index, String type, String query, Boolean bDetail)
		...
		SearchResponse response = getClient().prepareSearch(index).setTypes(type).setQuery(qb).setSize(500).execute()
				.actionGet();

		// Map of K,V pairs where key is the field name from search result and value is
		// the that should be returned for that field. Not always the same.
		Map<String, String> fieldsToReturn = new HashMap<>();

		fieldsToReturn.put("title", "Short Name");
//    fieldsToReturn.put("Dataset-LongName", "Long Name");
		fieldsToReturn.put("themekey", "Topic");

		System.out.println(fieldsToReturn.keySet());

		if (bDetail) {
			fieldsToReturn.put("abstract", "Dataset-Abstract");
			fieldsToReturn.put("purpose", "Dataset-Purpose");
			fieldsToReturn.put("direct", "DataFormat");
			fieldsToReturn.put("mapprojn", "Projection");
			fieldsToReturn.put("coordinates", "Coordinates");
...
//			fieldsToReturn.put("Dataset-AlongTrackResolution", "Dataset-AlongTrackResolution");
		...
		for (SearchHit hit : response.getHits().getHits()) {
			...
			// update hashmap to plain hierarchy

			Map<Object, Object> source_re = source.entrySet().stream().flatMap(FlatMap::flatten)
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue,
							(u, v) -> new IllegalStateException(String.format("Duplicate key %s", u)),
							LinkedHashMap::new));

			Map<String, Object> searchResult = source_re.entrySet().stream()
					.filter(entry -> fieldsToReturn.keySet().contains(entry.getKey()))
					.collect(Collectors.toMap(entry -> fieldsToReturn.get(entry.getKey()), Entry::getValue));

			// searchResult is now a map where the key = value from fieldsToReturn and the
			// value = value from search result
...
//      searchResult.put("Release Date", releaseDate.format(DateTimeFormatter.ISO_DATE));
			...
			if (bDetail) {
				...
				ArrayList<ArrayList<Double>> coors = (ArrayList<ArrayList<Double>>) searchResult.get("Coordinates");
...
//				List<String> urls = ((List<String>) searchResult.get("DatasetLocationPolicy-BasePath")).stream()
//						.filter(url -> url.startsWith("ftp") || url.startsWith("http")).collect(Collectors.toList());
//				searchResult.put("DatasetLocationPolicy-BasePath", urls);
//
//				// Time Span Formatting
//				LocalDate startDate = Instant
//						.ofEpochMilli((Long) searchResult.get("DatasetCoverage-StartTimeLong-Long"))
//						.atZone(ZoneId.of("Z")).toLocalDate();
//				LocalDate endDate = "".equals(searchResult.get("Dataset-DatasetCoverage-StopTimeLong")) ? null
//						: Instant
//								.ofEpochMilli(Long
//										.parseLong(searchResult.get("Dataset-DatasetCoverage-StopTimeLong").toString()))
//								.atZone(ZoneId.of("Z")).toLocalDate();
//				searchResult.put("Time Span", startDate.format(DateTimeFormatter.ISO_DATE) + " to "
//						+ (endDate == null ? "Present" : endDate.format(DateTimeFormatter.ISO_DATE)));
//
...
//					searchResult.put("SpatialResolution",
//							latResolution + " degrees (latitude) x " + lonResolution + " degrees (longitude)");
//				} else {
...
//					searchResult.put("SpatialResolution",
//							dAlonResolution + " km (Along) x " + dAcrossResolution + " km (Across)");
//				}
//
//				// Measurement is a list of hierarchies that goes Topic -> Term -> Variable ->
//				// Variable Detail. Need to construct these hierarchies.
//				List<List<String>> measurements = buildMeasurementHierarchies((List<String>) searchResult.get("Topic"),
//						(List<String>) searchResult.get("DatasetParameter-Term-Full"),
//						(List<String>) searchResult.get("DatasetParameter-Variable-Full"),
//						(List<String>) searchResult.get("DatasetParameter-VariableDetail"));
//
...
//
	 ...
	 * The hierarchy is built from the element in the same position from each input
	 * list in the order: Topic -> Term -> Variable -> VariableDetail "None" and
	 * blank strings are ignored. If, at any level, an element does not exist for
	 * that position or it is "None" or blank, that hierarchy is considered
	 * complete.
	 *
	 ...
	 * </pre>
	 * 
	 * Oceans > Sea Surface Topography > Sea Surface Height Oceans > Ocean Waves >
	 ...
	 */
	private List<List<String>> buildMeasurementHierarchies(List<String> topics, List<String> terms,
			List<String> variables, List<String> variableDetails) {
		...
		for (int x = 0; x < topics.size(); x++) {
			...
			if (x < terms.size() && !"None".equalsIgnoreCase(terms.get(x)) && StringUtils.isNotBlank(terms.get(x))) {
				...
				if (x < variables.size() && !"None".equalsIgnoreCase(variables.get(x))
					...
					if (x < variableDetails.size() && !"None".equalsIgnoreCase(variableDetails.get(x))
	...
	public List<String> autoComplete(String index, String term) {
		...
		Iterator<? extends Suggest.Suggestion.Entry.Option> iterator = sr.getSuggest().getSuggestion("completeMe")
